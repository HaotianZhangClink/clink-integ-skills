...
const WEBHOOK_ENSURE_COMMAND_SOURCE = String.raw`(?:\bclink\s+(?:webhook\s+endpoint|dashboard\s+webhook)\s+ensure\b|\bnode(?:\.exe)?\s+(?:"[^"\r\n]*clink-integ-cli[^"\r\n]*"|'[^'\r\n]*clink-integ-cli[^'\r\n]*'|[^\s;]*clink-integ-cli[^\s;]*)\s+(?:webhook\s+endpoint|dashboard\s+webhook)\s+ensure\b)`;
...
function extractWebhookEndpointEnsureCommands(input) {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    let match;
    while ((match = commandPattern.exec(lines[lineIndex])) !== null) {
      let command = lines[lineIndex].slice(match.index);
      let continuationIndex = lineIndex;

      while (continuationIndex + 1 < lines.length) {
        const currentLine = lines[continuationIndex].trimEnd();
        const nextLine = lines[continuationIndex + 1];
        if (!/[\\\x60]$/.test(currentLine) && !/^\s*--[a-z0-9-]+\b/i.test(nextLine)) break;
        continuationIndex += 1;
      ...
      const contexts = extractCommandContext(lines, lineIndex, continuationIndex);
      commands.push({
        command,

...
export function buildArtifacts({
  ...
  if (route === "merchant_standard_integration") {
    ...
    if (hasElementsSignals(prompt)) {
      artifacts.push(
        buildArtifact("elements_frontend_checklist", null, "Checklist for package or CDN choice, publish key, environment, session ID, mount containers, and lifecycle cleanup"),
        buildArtifact("elements_event_mapping", null, "Map submit-enabled, submit-visible, amount-change, session-init-success, session-success, session-pending, promo-code-error, and error into host UI state"),
        ...
        buildArtifact("elements_layout_recipe", null, hasElementsLayoutSignals(prompt)
      ...
      if (hasElementsPromotionSignals(prompt)) {
  ...
  if (route === "merchant_new_user_onboarding") {
    return [
      ...
      buildArtifact("dashboard_setup_checklist", null, "Dashboard setup checklist for Settings > Merchant, Settings > Users, Products, Developers > API Keys, and Developers > Webhooks"),
      buildArtifact("secret_setup_checklist", null, "Secret Key and webhook signing key retrieval paths with safe storage placeholders"),
...
export async function runSkillRuntime({
  ...
  if (route === "merchant_standard_integration") {
    ...
    if (hasElementsSignals(prompt) && hasClientOnlyFrameworkSignals(prompt)) {
      runtimeState = appendRuntimeNote(
        runtimeState,

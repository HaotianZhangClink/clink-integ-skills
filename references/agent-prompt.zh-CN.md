...
默认 GitHub 安装是轻量安装：使用 CLI 仓库已提交的 `dist/` 产物，不应触发安装期 TypeScript 构建，也不包含 Playwright 浏览器自动化。云 IDE、低代码、sandbox、CI 或已有 Secret Key 的接入路径不要安装 Playwright。
...
GitHub 安装应使用 CLI 仓库已提交的 `dist/` 产物；不要因为缺少 Node 类型声明就在业务项目里补 TypeScript 构建依赖；只有需要本地 `clink login` 时，才把 Playwright 安装到同一个 CLI tools prefix。

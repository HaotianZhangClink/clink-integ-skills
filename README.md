...
The default CLI install is lightweight and browser-free. If this local desktop path needs `clink login`, install optional Playwright support into the same tools prefix first:
...
After either path, product catalog import, checkout/subscription calls, webhook endpoint management, doctor, smoke-test, and local webhook commands should run with Secret Key authentication and should not require Playwright.
